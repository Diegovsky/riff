// Dialog showing album release metadata (label, release date, track count,
// copyright). Opened via the info button on the album detail page.
...
mod imp {
    ...
    /// GObject subclass backed by the `release_details.blp` template.
...
impl ReleaseDetailsDialog {
    ...
    /// Populate the dialog fields with album release metadata.
