...
fn setup_credits(about: &libadwaita::AboutDialog) {
    let authors: Vec<&str> = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/AUTHORS"))
        .trim_end_matches('\n')
        .split('\n')
        .collect();
    let translators =
        include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/TRANSLATORS")).trim_end_matches('\n');
    let artists: Vec<&str> = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/ARTISTS"))
        .trim_end_matches('\n')
        .split('\n')
        .collect();
...
/// Opens the browser to a pre-filled "new issue" page on the project's
/// GitHub tracker, with system information and a bug report template.
pub fn report_issue() {
