...
const IMAGE_FETCH_SIZE: i32 = 180;
...
impl CardWidget {
    ...
    fn bind(&self, model: &CardModel, worker: Worker) {
        ...
        *imp.card_id.borrow_mut() = model.id();
        ...
        let default_title = || gettext("Untitled");
        ...
        if let Some(url) = model.image() {
            ...
            worker.send_local_task(async move {
                if let Some(this) = weak.upgrade() {
                    ...
                    let pixbuf = loader
                        .load_remote(&url, "jpg", IMAGE_FETCH_SIZE, IMAGE_FETCH_SIZE)
                        .await;
            ...
            model
                ...
                .transform_to(|_, title: String| {
                    Some(if title.is_empty() { gettext("Untitled") } else { title })
                })
                .build();
            model
